...
const allCases = [
  ...
  { id: 'm3g-artifact', serveRoot: resolve(repositoryRoot, 'dist-site'), page: 'index.html', pass: 'M3G_SITE_SMOKE_PASS', fail: 'M3G_SITE_SMOKE_FAIL', flags: ['--use-angle=swiftshader', '--enable-unsafe-swiftshader', '--virtual-time-budget=10000'] },
  { id: 'm3i-selection-three', serveRoot: resolve(repositoryRoot, 'dist-site'), page: 'index.html?smoke=m3i', pass: 'M3I_SITE_SMOKE_PASS', fail: 'M3I_SITE_SMOKE_FAIL', flags: ['--use-angle=swiftshader', '--enable-unsafe-swiftshader', '--virtual-time-budget=10000'] },
  { id: 'm3i-selection-fallback', serveRoot: resolve(repositoryRoot, 'dist-site'), page: 'index.html?smoke=m3i&force-fallback=true', pass: 'M3I_SITE_SMOKE_PASS', fail: 'M3I_SITE_SMOKE_FAIL', flags: ['--disable-gpu', '--virtual-time-budget=10000'] }
];
...
const server = createServer((request, response) => {
  ...
  let relative;
  try { relative = pathname === '/' ? activeCase.page.split('?')[0] : decodeURIComponent(pathname.slice(1)); }
  catch { response.writeHead(400).end('bad request'); return; }
  const file = resolve(activeCase.serveRoot, normalize(relative));
  if (file !== activeCase.serveRoot && !file.startsWith(`${activeCase.serveRoot}${sep}`)) { response.writeHead(403).end('forbidden'); return; }
  try {
    const physicalRoot = realpathSync(activeCase.serveRoot); const physicalFile = realpathSync(file);
...
async function runCase(testCase, port) {
  ...
  const browser = spawn(chrome, ['--headless=new', '--no-sandbox', ...testCase.flags, '--dump-dom', url], { stdio: ['ignore', 'pipe', 'pipe'], detached: usesDetachedProcessGroup });
  let stdout = ''; let stderr = '';
  browser.stdout.setEncoding('utf8'); browser.stderr.setEncoding('utf8');
  browser.stdout.on('data', (chunk) => { stdout += chunk; }); browser.stderr.on('data', (chunk) => { stderr += chunk; });

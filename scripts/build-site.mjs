...
for (const [sourceRelative, destinationRelative] of copies) {
  ...
  assertSafeRelative(destinationRelative);
...
const lockfileSha256 = sha256(readFileSync(resolve(root, 'package-lock.json')));
const manifest = {
  schemaVersion: '1.0.0',
  sourceSha: head,
  lockfileSha256,
...
function assertSafeRelative(value) {
  if (value.length === 0 || value.startsWith('/') || value.split('/').some((part) => part === '' || part === '.' || part === '..')) throw new Error(`SITE_BUILD_INVALID_PATH:${value}`);
}

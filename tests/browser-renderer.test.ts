...
test('rejects missing, duplicate, and reordered manifest descriptors before preparation',()=>{for(const mutate of [(m:ProjectionManifestV2)=>({...m,nodeDescriptors:undefined}),(m:ProjectionManifestV2)=>({...m,nodeDescriptors:[m.nodeDescriptors[0]!,m.nodeDescriptors[0]!]}),(m:ProjectionManifestV2)=>({...m,nodeDescriptors:[...m.nodeDescriptors].reverse()})]){const base=project(scene,'html');const manifest=mutate(base) as unknown as ProjectionManifestV2;let preparedCalls=0,targetCalls=0;assert.throws(()=>renderProjection(manifest,{replaceChildren(){targetCalls+=1;}},{dom:{prepareHtml(){preparedCalls+=1;return prepared(base);},prepareSvg(){preparedCalls+=1;return prepared(base);}}}),/BROWSER_RENDER_INVALID_MANIFEST:nodeDescriptors:(type|duplicate|unsorted)/);assert.equal(preparedCalls,0);assert.equal(targetCalls,0);}});

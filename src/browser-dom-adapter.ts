...
const HTML_ATTRIBUTES = new Set(['id','href','tabindex','role','aria-label','data-node-id','data-edge-id','data-semantic-kind','data-source','data-target']);
...
function relationAttrs(element:Element):boolean{const actual=new Set(element.getAttributeNames().map((name)=>name.toLowerCase()));const base=['data-edge-id','data-semantic-kind'];const endpoint=['data-source','data-target'];if(actual.size===base.length&&base.every((name)=>actual.has(name)))return false;if(actual.size===base.length+endpoint.length&&[...base,...endpoint].every((name)=>actual.has(name)))return true;fail('html','attribute');}
...
function attrs(element:Element,kind:DomKind,allowed:readonly string[]):void{const set=new Set(allowed);for(const raw of element.getAttributeNames()){const name=raw.toLowerCase();if(name.startsWith('on')||name==='style'||!set.has(name))fail(kind,'attribute');}const attributeNames=new Set(element.getAttributeNames().map((name)=>name.toLowerCase()));if(attributeNames.size!==allowed.length||allowed.some((name)=>!attributeNames.has(name)))fail(kind,'attribute');}

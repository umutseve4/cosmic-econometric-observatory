...
function attrs(element:Element,kind:DomKind,allowed:readonly string[]):void{const set=new Set(allowed);for(const raw of element.getAttributeNames()){const name=raw.toLowerCase();if(name.startsWith('on')||name==='style'||!set.has(name))fail(kind,'attribute');}const names=new Set(element.getAttributeNames().map((name)=>name.toLowerCase()));if(names.size!==allowed.length||allowed.some((name)=>!names.has(name)))fail(kind,'attribute');}

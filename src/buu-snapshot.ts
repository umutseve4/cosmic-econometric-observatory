...
const TRANSFORMATION_VERSION = 'm1-buu-snapshot-v1';
...
export interface RawOffering {
  ...
  courseType: string; section: string; semester: number; instructor: string; classroom: string;
...
interface ParsedCurriculumRow { semester: number; courseCode: string; sourceTitle: string; courseType: 'required'|'elective'; theoryHours: number; practiceHours: number; labHours: number; ects: number; sourceLine: number; }
interface CatalogEntry { row: ParsedCurriculumRow; course: Course; relation: CurriculumRelation; anomalyRefs: string[]; }

export function parseBuuCurriculumTsv(text: string): ParsedCurriculumRow[] {
  let semester: number | null = null;
  const rows: ParsedCurriculumRow[] = [];
  ...
  if (rows.length !== 144 || rows.filter((row) => row.courseType === 'required').length !== 41 || rows.filter((row) => row.courseType === 'elective').length !== 103) throw new Error('Curriculum census drift');
  return rows;
}
...
export function importBuuSnapshot(curriculumTsv: string, inputOfferings: unknown): BuuSnapshot {
  ...
  const offeringBytes = Buffer.from(`${JSON.stringify(rawOfferings, null, 2)}\n`, 'utf8');
  assertHash(offeringBytes, OFFERINGS_HASH, 'offerings');
  const rows = parseBuuCurriculumTsv(curriculumTsv);
  const codeCounts = countBy(rows, (row) => row.courseCode);
  ...
  const catalog: CatalogEntry[] = rows.map((row) => {
    ...
    const duplicate = (codeCounts.get(row.courseCode) ?? 0) > 1;
    ...
    if (duplicate) {
      ...
      anomalyRefs.push(id);
      anomalies.push(anomaly(id, 'CONFLICTING_VALUE', `Duplicate official course code ${row.courseCode}; rows remain distinct.`, [courseId], { sourceLine: row.sourceLine, courseCode: row.courseCode, sourceTitle: row.sourceTitle }));
    }
    for (const [code, title, id] of suspiciousSpellings) if (row.courseCode === code && row.sourceTitle === title) {
      anomalyRefs.push(id);
      anomalies.push(anomaly(id, 'CONFLICTING_VALUE', `Suspicious source spelling preserved: ${title}`, [courseId], { sourceLine: row.sourceLine, sourceTitle: title }));
    }
    const provenance = curriculumProvenance(row.sourceLine);
    const course: Course = { id: courseId, kind: 'course', canonicalTitle: row.sourceTitle, codeAssignments: [{ value: row.courseCode, validFrom: '2025-09-01' }], provenance };
    const relation: CurriculumRelation = { id: `cc-${courseId}`, kind: 'curriculum-relation', curriculumVersionId: CURRICULUM_ID, courseId, semester: row.semester, status: row.courseType, ects: row.ects, ...(row.courseType === 'elective' ? { poolId: `elective-s${row.semester}` } : {}), provenance };
    return { row, course, relation, anomalyRefs };
  });
  anomalies.push(anomaly('anomaly-curriculum-ay33-total-ects', 'CONFLICTING_VALUE', 'Official program metadata reports 240 ECTS while published semester totals sum to 241 ECTS; neither value was repaired.', [CURRICULUM_ID], { programTotalEcts: 240, semesterTotals: [31,30,30,30,30,30,30,30], semesterTotalEcts: 241 }));

  const unresolvedCourses = new Map<string, Course>();
  const reconciliations: Reconciliation[] = [];
  const offerings: Offering[] = rawOfferings.map((raw) => {
    ...
    if (!courseId) {
      ...
      if (!unresolvedCourses.has(courseId)) unresolvedCourses.set(courseId, { id: courseId, kind: 'course', canonicalTitle: raw.sourceTitle, codeAssignments: [{ value: raw.printedCourseCode, validFrom: '2025-09-01' }], provenance: offeringProvenance(raw) });
    }
    for (const ref of reconciliation.anomalyRefs.filter((ref) => ref === 'offering-printed-code-mismatch')) {
      const id = `${ref}:${raw.id}`;
      reconciliation.anomalyRefs = reconciliation.anomalyRefs.map((value) => value === ref ? id : value);
      anomalies.push(anomaly(id, 'CONFLICTING_VALUE', `Printed code ${raw.printedCourseCode} conflicts with unique title match ${raw.sourceTitle}; printed values were preserved.`, [raw.id, courseId], { sourceRowId: raw.sourceRowId, printedCode: raw.printedCourseCode, printedTitle: raw.sourceTitle }));
    }
    return { id: raw.id, kind: 'offering', courseId, academicYear: raw.academicYear, instructionType: raw.educationType === 'first' ? 'primary' : 'secondary', section: raw.section, ...(raw.instructor ? { instructorIds: [`instructor-source:${shortHash(raw.instructor)}`] } : {}), schedule: [{ day: weekday(raw.weekday), startsAt: raw.startTime, endsAt: raw.endTime, ...(raw.classroom ? { room: raw.classroom } : {}) }], provenance: offeringProvenance(raw) };
  });
  const allCourses = [...catalog.map((entry) => entry.course), ...unresolvedCourses.values()];
  validateReferences(allCourses, catalog.map((entry) => entry.relation), offerings, reconciliations, anomalies);
  const summary = {
    relations: catalog.length, required: rows.filter((row) => row.courseType === 'required').length, elective: rows.filter((row) => row.courseType === 'elective').length,
    offerings: rawOfferings.length, spring: rawOfferings.filter((row) => row.term === 'spring').length, fall: rawOfferings.filter((row) => row.term === 'fall').length,
    primary: rawOfferings.filter((row) => row.educationType === 'first').length, secondary: rawOfferings.filter((row) => row.educationType === 'second').length,
    mapped: reconciliations.filter((row) => row.status === 'mapped').length, 'mapped-with-anomaly': reconciliations.filter((row) => row.status === 'mapped-with-anomaly').length,
    ambiguous: reconciliations.filter((row) => row.status === 'ambiguous').length, unmatched: reconciliations.filter((row) => row.status === 'unmatched').length
  } as const;
  if (JSON.stringify(summary) !== JSON.stringify({ relations:144,required:41,elective:103,offerings:164,spring:83,fall:81,primary:108,secondary:56,mapped:129,'mapped-with-anomaly':15,ambiguous:0,unmatched:20 })) throw new Error(`Snapshot partition drift: ${JSON.stringify(summary)}`);
  return Object.freeze({ courses: Object.freeze(allCourses), curriculumRelations: Object.freeze(catalog.map((entry) => entry.relation)), offerings: Object.freeze(offerings), reconciliations: Object.freeze(reconciliations), anomalies: Object.freeze(anomalies), summary: Object.freeze(summary) });
}
...
function typeOf(value: string): 'required'|'elective'|null { return value === 'Z' ? 'required' : value === 'S' ? 'elective' : null; }
...
function curriculumProvenance(line: number): Provenance { return { sourceId: 'source:buu-program-package', snapshotId: 'snapshot:buu-program-ay33-2026-08-23', locator: `program-343-ay33.rows.tsv#line=${line}`, observedAt: OBSERVED_AT, contentHash: `sha256:${CURRICULUM_HASH}`, transformationVersion: TRANSFORMATION_VERSION }; }
function offeringProvenance(row: RawOffering): Provenance { return { sourceId: 'source:buu-timetable', snapshotId: row.sourceSnapshotId, locator: `offerings.json#${row.sourceRowId}`, observedAt: OBSERVED_AT, contentHash: `sha256:${OFFERINGS_HASH}`, transformationVersion: TRANSFORMATION_VERSION }; }
function anomaly(id: string, code: Anomaly['code'], message: string, entityRefs: string[], evidence: unknown): Anomaly { return { id, code, severity: 'warning', message, entityRefs, evidence: [evidence] }; }
...
function validateReferences(courses: Course[], relations: CurriculumRelation[], offerings: Offering[], reconciliations: Reconciliation[], anomalies: Anomaly[]): void {
  ...
  const courseIds = unique(courses.map((x) => x.id), 'course'), relationIds = unique(relations.map((x) => x.id), 'relation'); unique(offerings.map((x) => x.id), 'offering'); unique(anomalies.map((x) => x.id), 'anomaly');
